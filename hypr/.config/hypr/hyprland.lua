-- Hyprland config (Lua, the format that replaces hyprland.conf from 0.57).
-- Companion files in this directory: hypridle.conf, hyprlock.conf (those stay hyprlang).
-- Reference: https://wiki.hypr.land/Configuring/
-- Type stubs for editor completion: /usr/share/hypr/stubs/hl.meta.lua
...
-- Commands run through a shell, so $HOME expands.
hl.on("hyprland.start", function()
    hl.exec_cmd("waybar -c $HOME/.config/waybar/top.jsonc")    -- Top status bar.
    hl.exec_cmd("waybar -c $HOME/.config/waybar/bottom.jsonc") -- Bottom status bar.
    hl.exec_cmd("mako")                                        -- Notification daemon
    hl.exec_cmd("wl-clip-persist --clipboard regular")         -- Keep clipboard after the source app closes
    ...
    hl.exec_cmd("signal-desktop")                              -- Signal IM
    hl.exec_cmd("kanshi")                                      -- Monitors manager
end)
...
-- Laptop lid. logind ignores the lid (see /etc/systemd/logind.conf), so Hyprland drives it.
-- Switch name from `hyprctl devices`. Policy lives in ~/.local/bin/lid-handler.sh (hyprlid package).
hl.bind("switch:on:Lid Switch",  hl.dsp.exec_cmd("~/.local/bin/lid-handler.sh"), { locked = true })
hl.bind("switch:off:Lid Switch", hl.dsp.exec_cmd("~/.local/bin/lid-handler.sh"), { locked = true })
